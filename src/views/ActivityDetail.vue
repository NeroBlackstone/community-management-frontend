        ...
        <div v-if="data">
            <v-stepper >
                <v-stepper-header v-if="data.activity.status==='PENDING'">
                    <v-stepper-step step="1" complete>已提交</v-stepper-step>
                    <v-divider></v-divider>
                    <v-stepper-step step="2">等待回复</v-stepper-step>
                </v-stepper-header>
                <v-stepper-header v-else>
                    <v-stepper-step step="1" complete>已提交</v-stepper-step>
                    <v-divider></v-divider>
                    <v-stepper-step v-if="data.activity.status==='APPROVED'" step="2" complete>已通过</v-stepper-step>
                    <v-stepper-step v-else step="2" :rules="[() => false]">Rejected</v-stepper-step>
                </v-stepper-header>
            </v-stepper>
            <v-card class="mx-auto" color="primary" dark max-width="800">
                ...
                <v-card-text class="headline font-weight-bold">活动内容：{{data.activity.content}}</v-card-text>
                <v-card-text class="headline font-weight-bold">地点：{{data.activity.place}}</v-card-text>
                <v-card-text class="headline font-italic">
                    开始于: {{Date.create(data.activity.startAt).format('{yyyy}-{MM}-{dd} {HH}:{mm}')}}
                </v-card-text>
            </v-card>
